...
void ConfigHandler::fix_from_0_1_18()
{
	...
	while(hit)
	{
		...
		{
			hit=true;
			if(!findChar(l.at(i).nodeName(), tmpNode)) createChar(l.at(i).nodeName());
...
bool ConfigHandler::findChar(QString name, QDomNode &node)
{
...
{
	QDomNode tmpNode;
	if(! findChar(v.name, tmpNode)) createChar(v.name);
